...
def animate_race(data):
    ...
    ax.set_ylim(0, numeric_max + 10)  # 여유를 주기 위해 y축 범위를 더 늘림

    # x축과 y축 라벨 설정
    ax.set_xlabel("count")
    ax.get_yaxis().set_visible(False)  # y축 숫자 라벨 제거
    ...
    def update(frame):
        ...
        if frame == num_frames - 1:
            top_3_current = current_data.groupby('person')['count'].last().nlargest(1)
            for rank, (person, count) in enumerate(top_3_current.items(), start=1):
                ax.text(num_frames * 0.8, numeric_max -  rank, 
                        f"{rank}위: {person}({count})", 
                        fontsize=30, ha='right', color='black')
